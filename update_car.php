<?php
	require_once('dbconnect.php');
		...
		$tag = filter_input(INPUT_POST, 'update_tag');

		$sql = "SELECT * FROM car_info WHERE tag_number = '$tag'";
		$result = $conn->query($sql);
		if($result->num_rows < 1){
			$error = "TAG DOES NOT EXIST OR IS ARCHIVED AND MUST BE REINSTATED";
			include('error.php');
		}
	$conn->close();
...
<html>
	...
	<body>
        ...
        <form class="text-center form-horizontal" action="updated_car.php" method="post" style="margin: 20px;">
			...
			<div class="col-md-5 col-md-push-2">
				<input class="form-control" type="text" name="location" value="<?php echo ($row['location']); ?>" required><br>
			</div>
			</div>
			...
			<div class="col-md-4 col-md-push-2">
				<input type="submit" name="update" value="Update Car" id="add"><br><br>
				<input type="submit" name="up-arch" value="Update and Archive Car"><br>
			</div>
			</div> 
		</form>
